...
# Data for the plot
means = np.array([
    [-0.2, 0.11],  # No adjustment (as decimal)
    # [-0.15, 0.10],  # Adjustment with benchmark
    [-0.045, 0.067]   # Adjustment with FAF DL model
])
errors = np.array([
    [0.19, 0.19],  # Confidence intervals for No adjustment
    # [0.03, 0.02],  # Confidence intervals for Adjustment with benchmark
    [0.1, 0.1]   # Confidence intervals for Adjustment with FAF DL model
])
...
y_positions = np.arange(len(adjustments))[::-1] - 0.5
...
for col in range(2):
    for i, (mean, error) in enumerate(zip(means[:, col], errors[:, col])):
        ...
        xstr = remove_0(str(np.abs(mean) * 100))
        ax[col].text(mean, y_positions[i] + 0.2, f"{xstr}%", va='center', ha='center', fontsize=25)
    ...
    ax[col].set_xticklabels([x for x in [-2, -1, 0, 1, 2]], fontsize=20)
...
ax[0].set_yticks(y_positions[:-1])
ax[0].set_yticklabels(adjustments[:-1], fontsize=12)
ax[1].set_yticks(y_positions[:-1])
