...
AIREADI_DEVICE_PLOT_NAME = {
    "Spectralis": "AI-READI Heidelberg Spectralis",
    ...
    for i, plot_task in enumerate(plot_tasks):
        ...
        for j, m in enumerate(plot_methods):
            ...
            if err_bar:
                ...
                ax.errorbar((j + 1)*width, y, yerr=y_std_err, fmt='none', ecolor='gray', capsize=5, zorder=4)
        ...
        y_h_1 = df_dict[plot_task][plot_methods[2]][:, plot_col_idx].tolist()
        y_l_1 = df_dict[plot_task][plot_methods[-1]][:, plot_col_idx].tolist()

        p_value_1 = ttest_rel(y_h_1*2 , y_l_1*2)[1]
        x1 = (2 + 1) * width
        x2 = (5 + 1) * width + 0.01
        delta_y = 0.01
        line_y1 = np.mean(y_h_1) + np.std(y_h_1) / np.sqrt(len(y_h_1)) + delta_y
        stars = get_star_from_pvalue(p_value_1, star=True)
        if np.mean(y_h_1) > np.mean(y_l_1) and len(stars) > 0:
            ax.plot([x1, x1], [np.mean(y_h_1) + np.std(y_h_1)/np.sqrt(len(y_h_1)) + 0.5*delta_y, line_y1], c=ax.spines['bottom'].get_edgecolor(), linewidth=1)
            ax.plot([x2, x2], [np.mean(y_l_1) + np.std(y_l_1)/np.sqrt(len(y_l_1)) + 0.5*delta_y, line_y1], c=ax.spines['bottom'].get_edgecolor(), linewidth=1)
            ax.plot([x1, x2], [line_y1, line_y1], c=ax.spines['bottom'].get_edgecolor(), linewidth=1)
            ax.text((x1 + x2)/2, line_y1, stars, fontsize=7, ha='center', va='bottom')        

        y_h_2 = df_dict[plot_task][plot_methods[-2]][:, plot_col_idx].tolist()
        y_l_2 = df_dict[plot_task][plot_methods[-1]][:, plot_col_idx].tolist()
        print(y_h_2, y_l_2)
        p_value_2 = ttest_rel(y_h_2*3 , y_l_2*3)[1]
        print(f'{plot_task}: {p_value}', p_value_1 , p_value_2)
        line_y2 = np.mean(y_h_2) + np.std(y_h_2) / np.sqrt(len(y_h_2)) + delta_y
        stars = get_star_from_pvalue(p_value_2, star=True)
        x1 = (4 + 1) * width
        x2 = (5 + 1) * width
        if np.mean(y_h_2) > np.mean(y_l_2) and len(stars) > 0:
            ax.plot([x1, x1], [np.mean(y_h_2) + np.std(y_h_2)/np.sqrt(len(y_h_2)) + 0.5*delta_y, line_y2], c=ax.spines['bottom'].get_edgecolor(), linewidth=1)
            ax.plot([x2-0.01, x2-0.01], [np.mean(y_l_2) + np.std(y_l_2)/np.sqrt(len(y_l_2)) + 0.5*delta_y, line_y2], c=ax.spines['bottom'].get_edgecolor(), linewidth=1)
            ax.plot([x1, x2-0.01], [line_y2, line_y2], c=ax.spines['bottom'].get_edgecolor(), linewidth=1)
            ax.text((x1 + x2)/2, line_y2, stars, fontsize=7, ha='center', va='bottom')     
        # exit()
...
if __name__ == '__main__':
    ...
    plt.savefig(os.path.join(save_file_dir, 'save_figs', 'figure_4g-i_new.pdf'), dpi=300)
    plt.savefig(os.path.join(save_file_dir, 'save_figs', 'figure_4g-i_new.png'))
