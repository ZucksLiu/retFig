...
print(len(image_index), len(original_img))
# exit()
num_frames = 1
# plot
fig, axs = plt.subplots(3, num_frames, figsize=(4, 10))
if patient_idx == 8490:
    ...
    if patient_idx == 849:
        region = [(170, 100, 80, 60), (290, 120, 80, 60), (100, 100, 80, 60)]
    ...
    for i in range(num_frames):
        if num_frames == 1:
            ...
            ax_idx = (0, i)
        axs[ax_idx].imshow(original_img[i][region[i][1]:region[i][1]+region[i][3], region[i][0]:region[i][0]+region[i][2]])
        axs[ax_idx].axis('off')
        axs[ax_idx].set_title(f'Slice {image_index[i]}, (probability: {pos_prob_list[patient_idx][image_index[i]]:.2f})', fontsize=20)
        if num_frames == 1:
            ax_idx = 1
        else:
            ax_idx = (1, i)

        axs[ax_idx].imshow(original_img[i+num_frames][region[i+num_frames][1]:region[i+num_frames][1]+region[i+num_frames][3], region[i+num_frames][0]:region[i+num_frames][0]+region[i+num_frames][2]])
        axs[ax_idx].axis('off')
        axs[ax_idx].set_title(f'Slice {image_index[i+num_frames]}, (probability: {pos_prob_list[patient_idx][image_index[i+num_frames]]:.2f})', fontsize=20)
        if num_frames == 1:
            ax_idx = 2
        else:
            ax_idx
        axs[ax_idx].imshow(original_img[i+2*num_frames][region[i+2*num_frames][1]:region[i+2*num_frames][1]+region[i+2*num_frames][3], region[i+2*num_frames][0]:region[i+2*num_frames][0]+region[i+2*num_frames][2]])
        axs[ax_idx].axis('off')
        axs[ax_idx].set_title(f'Slice {image_index[i+2*num_frames]}, (probability: {pos_prob_list[patient_idx][image_index[i+2*num_frames]]:.2f})', fontsize=20)
...
plt.tight_layout()
plt.savefig(save_dir + f'diabete_show_slice_{patient_idx}_2x3_amp.png')
plt.savefig(save_dir + f'diabete_show_slice_{patient_idx}_2x3_amp.pdf', dpi=300)
plt.show()