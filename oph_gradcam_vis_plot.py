...
num_frames = 9
# plot
fig, axs = plt.subplots(4, num_frames, figsize=(20, 9))
axs[0, 0].set_ylabel('IR en face image (w/ AMD)')
axs[1, 0].set_ylabel('Raw OCT frame')
axs[2, 0].set_ylabel('RETFound saliency')
axs[3, 0].set_ylabel('OctCube saliency')
for i in range(num_frames - 2):
    axs[0, i].set_title(f'Frame {image_index[i]}')#, dim (y, z)')
...
fig.tight_layout(pad=0.5, h_pad=1.0, w_pad=1.0, rect=[0, 0, 0.95, 1])  # Leave space on the right


# Add an additional axis at the right of the figure for the colorbar
cbar_ax = fig.add_axes([0.96, 0.15, 0.02, 0.7])  # Adjust the left value to prevent overlapping
...
cbar.ax.set_xlabel('Saliency', labelpad=10, rotation=0)
...
fig.add_artist(line)
# fig.tight_layout()
plt.savefig(save_dir + 'gradcam_vis_plot.jpg')
plt.savefig(save_dir + 'gradcam_vis_plot.pdf', format='pdf', dpi=300)
