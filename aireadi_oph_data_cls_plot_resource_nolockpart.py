...
AIREADI_EVAL_FT = {
    "MAE-joint": ["finetune",  "linear_probe"], # "lock_part",
    "retfound": ["finetune", "linear_probe"], # "lock_part", 
    # "MAE2D": ["3D"],
    # "linear_probe": ["3D"],
    # "unlock_top": ["3D"],
...
if __name__ == '__main__':
    ...
    # Plot the figure
    fig, axes = plt.subplots(figsize=(2*FIG_WIDTH, 1*FIG_HEIGHT), nrows=1, ncols=2)
    ...
    # AIREADI_oph_tasks_barplot(fig, axes[0], grouped_dict, setting_code='default', plot_col='auprc', plot_tasks=[], plot_methods= [('MAE-joint', '3D', 'finetune'), ('retfound', '3D', 'finetune'), ('MAE-joint', '3D', 'linear_probe'),  ('retfound', '3D', 'linear_probe')], y_name='AUPRC')
    # plot the subfigure f-j
    all_handles, all_labels = AIREADI_oph_tasks_barplot(fig, axes[0], grouped_dict, setting_code='default', plot_col='auroc', plot_tasks=[], plot_methods=[('MAE-joint', '3D', 'finetune'), ('retfound', '3D', 'finetune'), ('MAE-joint', '3D', 'linear_probe'),  ('retfound', '3D', 'linear_probe')], y_name='AUROC')
    AIREADI_oph_memory_barplot(fig, axes[1], grouped_dict, gpu_memory_dict, setting_code='default', plot_tasks=[], plot_methods=[('MAE-joint', '3D', 'finetune'), ('retfound', '3D', 'finetune'), ('MAE-joint', '3D', 'linear_probe'),  ('retfound', '3D', 'linear_probe')], y_name='GPU memory usage', err_bar=False)
    ...
    plt.savefig(os.path.join(save_file_dir, 'save_figs', 'aireadi_figure_resource_used.pdf'), dpi=300)
    plt.savefig(os.path.join(save_file_dir, 'save_figs', 'aireadi_figure_resource_used.png'))
