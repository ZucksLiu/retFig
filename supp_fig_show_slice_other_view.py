...
num_frames = 5
# plot
fig, axs = plt.subplots(3, num_frames, figsize=(20, 9))
for i in range(num_frames):
    axs[0, i].imshow(original_img[i])
    axs[0, i].axis('off')
    axs[0, i].set_title(f'Slice {image_index[i]}, disease probability: {pos_prob_list[patient_idx][image_index[i]]:.3f}', fontsize=15)
    axs[1, i].imshow(original_img[i+num_frames])
    axs[1, i].axis('off')
    axs[1, i].set_title(f'Slice {image_index[i+num_frames]}, disease probability: {pos_prob_list[patient_idx][image_index[i+num_frames]]:.3f}', fontsize=15)
    axs[2, i].imshow(original_img[i+2*num_frames])
    axs[2, i].axis('off')
    axs[2, i].set_title(f'Slice {image_index[i+2*num_frames]}, disease probability: {pos_prob_list[patient_idx][image_index[i+2*num_frames]]:.3f}', fontsize=15)
...
if patient_idx == 4:
    ...
    idx = 2 
fig.suptitle(f'AMD patient {idx}', fontsize=20)
plt.tight_layout()
plt.savefig(save_dir + f'show_slice_{patient_idx}.png')
plt.savefig(save_dir + f'show_slice_{patient_idx}.pdf', dpi=300)
plt.show()